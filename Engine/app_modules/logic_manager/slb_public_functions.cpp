...
void SLBPublicFunctions::playerTalks(const char* text, const float ttl) {
  // DO Something with text...
  dbg(text);
  dbg(to_string(ttl).c_str());
  ...
  MKeyValue atts3;
  atts3["text"] = text;
  atts3["ttl"] = to_string(ttl).c_str();
  new_hl.load(atts3);
  entity->add(new_hl);
}

void SLBPublicFunctions::playerTalksWithColor(const char* text, const float ttl, const char* background, const char* textColor) {
  // DO Something with text...
  dbg(text);
  dbg(to_string(ttl).c_str());
