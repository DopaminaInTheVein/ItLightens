...
Texture2D txSpecular : USE_SHADER_REG(TEXTURE_SLOT_SPECULAR_GL);
	...
	) : SV_Target
{
	...
	/*if (inv_shadows >= limit) {
		...
		//c = txHatch6.Sample(samLinear, pixel_pos);
		c = lerp(tx_test1.Sample(samLinear, pixel_pos), float4(1.0f,1.0f,1.0f,1.0f), 2.0f * (shading - 2. * inv_shadows));
	}
	...
	float color = inv_shadows - 1;
	//c.rgb = float3(color,color,color);		//lines black
	//float w = c.r;
	//w = step(w, 0.60f);
	//w = 1 -w;
	//w *= 0.2f;
